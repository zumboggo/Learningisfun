...
export function PlannerLessonPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-4 p-4 sm:p-6">
    ...
    <div className="grid gap-4 md:grid-cols-2">
      <Card><h2 className="font-semibold">Scheduled for this lesson</h2><p className="mt-2 text-sm"><strong>Texts:</strong> {lesson.texts.join(' · ') || 'None selected'}</p><p className="mt-2 text-sm"><strong>Presentations:</strong> {lesson.presentations.join(' · ') || 'None selected'}</p><p className="mt-2 text-sm"><strong>Due:</strong> {lesson.due.join(' · ') || 'Nothing listed'}</p>{course?.weDoLead === 'named' && <p className="mt-2 text-sm"><strong>WE DO lead:</strong> {course.leadName || 'Name not entered'}</p>}</Card>
