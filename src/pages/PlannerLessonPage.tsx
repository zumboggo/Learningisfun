...
export function PlannerLessonPage() {
  ...
  return <div className="mx-auto max-w-4xl space-y-4 p-4 sm:p-6">
    ...
    {lesson.slots&&<WeeklySlots data={{...data,lessons:[lesson]}} units={data.unitSnapshot||[]} onChange={next=>setLesson(next.lessons[0])}/>}<div className="space-y-3">{(!lesson.slots?fields:[]).map(([key, label]) => <Card key={key}><div className="grid gap-3 sm:grid-cols-[7rem_1fr]"><span className="text-xs font-bold uppercase tracking-wide text-blue-700">{label}</span><textarea className="w-full rounded-lg border p-3 text-sm" rows={key === 'weDo' ? 4 : 3} value={String(lesson[key] || '')} onChange={event => update(key, event.target.value)} /></div></Card>)}</div>
