...
export function DiscussionComposer({storageKey,category,busy,onSubmit,onCancel}:{storageKey:string;category:ReadingCategory;busy:boolean;onSubmit:(fields:Record<string,unknown>)=>Promise<void>;onCancel?:()=>void}) {
  ...
  return <form className="my-4 space-y-3 rounded-2xl border border-slate-200 bg-slate-50 p-4" onSubmit={e=>{e.preventDefault();void onSubmit(draft).then(()=>{setDraft(blank());try{localStorage.removeItem(storageKey);}catch{setStorageError('Posted successfully, but the old local draft could not be cleared.');}}).catch(()=>{});}}>
    <label className="block text-sm font-medium">{onCancel?'Your reply':`Add to ${readingCategories[category]}`}<textarea aria-label={onCancel?'Your reply':'Your contribution'} className="mt-2 w-full rounded-xl border bg-white p-3 text-base text-slate-900" rows={3} maxLength={10000} required value={draft.content} placeholder={onCancel?'Build on this idea…':prompts[category]} onChange={e=>update('content',e.target.value)}/></label>
...
function ReadingThread({post,posts,teacher,canWrite,busy,mutate,readUrl,draftPrefix,present,onSelect,depth=0,ancestorLocked=false}:{post:ReadingDiscussionPost;posts:ReadingDiscussionPost[];teacher:boolean;canWrite:boolean;busy:boolean;mutate:(action:string,fields:Record<string,unknown>)=>Promise<void>;readUrl:string;draftPrefix:string;present:boolean;onSelect:()=>void;depth?:number;ancestorLocked?:boolean}) {
  ...
  return <article className={`rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 ${post.hidden?'opacity-60':''}`}>
    ...
    <div className="mt-3 flex flex-wrap items-center gap-2 text-sm">
      <button disabled={!canWrite||busy||post.hidden} title={post.category==='question'?'I’d like us to discuss this':'Upvote'} aria-label={`${post.voted?'Remove upvote':'Upvote'}: ${post.score}`} aria-pressed={post.voted} className={`min-h-11 rounded-lg border px-3 ${post.voted?'bg-blue-50 text-blue-800':''}`} onClick={()=>send('voteReadingDiscussion',{upvoted:!post.voted})}>↑ {post.score}</button>
      ...
      {teacher&&!present&&(['pin','hide','lock'] as const).map(operation=>{const active=operation==='pin'?post.pinned:operation==='hide'?post.hidden:post.locked;const action=active?({pin:'unpin',hide:'show',lock:'unlock'} as const)[operation]:operation;return <button disabled={busy} key={operation} className="min-h-11 px-2 capitalize text-slate-600" onClick={()=>send('moderateReadingDiscussion',{operation:action})}>{action}</button>;})}
    </div>
    ...
    {reply&&!locked&&<DiscussionComposer storageKey={`${draftPrefix}:reply:${post.id}`} category={post.category} busy={busy} onCancel={()=>setReply(false)} onSubmit={fields=>mutate('postReadingDiscussion',{...fields,parentId:post.id}).then(()=>setReply(false))}/>}
