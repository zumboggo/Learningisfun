...
function DiscussionWorkspace({textId,classId}:{textId:string;classId:string}) {
  ...
  return <div className={present?'fixed inset-0 z-50 overflow-auto bg-white p-6 sm:p-12':'mx-auto max-w-4xl space-y-5 p-4 sm:p-6'}>
    ...
    {!present&&<p className="text-sm text-slate-500">{data.showStudentNames?'Classmates can see usernames on posts and replies.':'Posts and replies are anonymous to classmates. Your teacher can see who wrote them.'}</p>}
