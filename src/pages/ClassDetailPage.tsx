  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 p-4 sm:space-y-6 sm:p-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        ) : <div className="flex flex-wrap gap-2"><Button size="sm" variant="secondary" onClick={()=>setShowClassLinks(true)}>Class Links</Button><Button size="sm" variant="secondary" loading={refreshingMaterials} onClick={() => void refreshMaterials()}>↻ Refresh class</Button></div>}
        <Modal open={showClassLinks} onClose={()=>setShowClassLinks(false)} title="Class Links"><div className="space-y-2">{parseClassLinks(cls.linksJson).map(link=><a key={link.url+link.label} href={link.url} target="_blank" rel="noopener noreferrer" className="block rounded-xl border p-3 text-blue-700 underline">{link.label} ↗</a>)}{!parseClassLinks(cls.linksJson).length&&<p>No class links have been added yet.</p>}</div></Modal>
      </div>
      ...
      {isOwner && <FlashcardReportsPanel classId={cls.$id} />}
      ...
      <details className="overflow-hidden rounded-xl border border-gray-200 bg-white">
        <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-4 hover:bg-gray-50">
          <span><strong className="block text-gray-950">{isOwner ? 'Class resources' : 'More class resources'}</strong><span className="text-sm text-gray-500">{isOwner ? 'Links, quizzes, discussions, and card decks' : 'Reference links, discussion history, and card decks'}</span></span>
      ...
      {isOwner && <section id="quizzes" className="scroll-mt-4">
...
function FlashcardReportsPanel({classId}:{classId:string}){const[reports,setReports]=useState<Array<{id:string;deckTitle:string;front:string;studentName:string;reason:string;createdAt:string}>>([]);useEffect(()=>{void listFlashcardReports(classId).then(result=>setReports(result.reports)).catch(()=>undefined)},[classId]);if(!reports.length)return null;return <details className="overflow-hidden rounded-xl border border-amber-200 bg-amber-50"><summary className="flex cursor-pointer list-none items-center justify-between px-4 py-3 font-semibold text-amber-950"><span>Reported flashcards</span><span className="rounded-full bg-white px-2 py-1 text-xs">{reports.length}</span></summary><div className="space-y-2 border-t border-amber-200 p-3">{reports.map(report=><article key={report.id} className="rounded-lg bg-white p-3 text-sm"><strong>{report.deckTitle} · {report.front}</strong><p className="mt-1 text-gray-700">{report.reason}</p><div className="mt-2 flex items-center justify-between gap-3"><p className="text-xs text-gray-500">Reported by {report.studentName} · {new Date(report.createdAt).toLocaleDateString()}</p><button className="text-xs font-semibold text-emerald-700" onClick={()=>void resolveFlashcardReport(report.id).then(()=>setReports(current=>current.filter(item=>item.id!==report.id)))}>Resolved</button></div></article>)}</div></details>}
  ...
  const save = async () => {
    ...
    try {
      for (const text of texts || []) {
        ...
        await updateTextAssignments(text.$id, teacherId, [...classIds].map(id=>{const prior=current.find(a=>a.classId===id);return id===classId?{classId:id,...purpose,...(prior&&!dueDate?{assignedAt:prior.assignedAt,dueDate:prior.dueDate}:textSchedule(dueDate,prior?.assignedAt))}:{classId:id,assignedAt:prior!.assignedAt,dueDate:prior!.dueDate};}));
      }
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-gray-200 bg-white">
      <button className="flex w-full items-center justify-between p-4 text-left hover:bg-gray-50" onClick={() => setOpen(value => !value)}>
        <span className="text-lg font-semibold">{open ? '▾' : '▸'} Weekly links</span>
      ...
      {open && <div className="space-y-3 border-t p-4">
        ...
        <p className="text-sm text-gray-500">Links saved across class weeks. Add a link from the week where students should find it.</p>
        <div className="max-h-80 space-y-2 overflow-y-auto pr-1">
          {links.length ? links.map(link => <div key={link.$id} className="flex items-center gap-3 rounded-lg border p-3">
            <label className="flex min-w-0 flex-1 items-center gap-3">
              ...
              <span className="min-w-0">{link.url.startsWith('presentation-file:')?<PresentationDownload id={link.$id} className="block truncate font-medium text-blue-700 hover:underline">{link.title}</PresentationDownload>:<a href={link.url || PRESENTATION_FOLDER_URL} target="_blank" rel="noreferrer" className="block truncate font-medium text-blue-700 hover:underline" onClick={event => event.stopPropagation()}>{link.title}</a>}<span className="text-xs text-gray-500">{link.watchedAt ? 'Watched' : 'Not watched yet'} · {formatDate(link.assignedAt)}</span></span>
            </label>
            {isOwner && <button className="text-sm text-red-600" onClick={() => window.confirm('Delete this link?') && void deletePresentationLink(link.$id)}>Delete</button>}
          </div>) : <p className="rounded-lg bg-gray-50 p-4 text-sm text-gray-500">No weekly links yet. Open a week below to add one.</p>}
        </div>
      </div>}
    </section>
  );
...
function WeeklyClassMaterials({ classId, materials, isOwner, onOpenQuizResults }: { classId: string; materials: WeeklyMaterial[]; isOwner: boolean; onOpenQuizResults: (quiz: Quiz) => void }) {
  ...
  const saveQuickAdd = async () => {
    ...
    if (quickAdd.kind==='link' && !url.trim()) { setAddError('Enter the URL for this link.'); return; }
        ...
        {visibleWeeks.map(([week, unsortedItems]) => {
          ...
          return <div key={week} className="overflow-hidden rounded-xl border border-gray-200 bg-white">
            ...
            {isOpen && <div className="space-y-3 border-t bg-gray-50 p-4">
              ...
              {presentations.length > 0 && <CompactWeekSection title="Links" count={presentations.length} color="fuchsia" open={openSections.has(`${week}-presentations`)} onToggle={() => setOpenSections(current => toggleSetValue(current, `${week}-presentations`))}>{presentations.map(item => <div key={item.presentation.$id} className="flex items-center gap-3 border-t border-fuchsia-100 px-4 py-2.5"><input aria-label={`Mark ${item.presentation.title} viewed`} type="checkbox" className="h-5 w-5" checked={Boolean(item.presentation.watchedAt)} disabled={!isOwner} onChange={event => void setPresentationWatched(item.presentation.$id, event.target.checked)} />{item.presentation.url.startsWith('presentation-file:')?<PresentationDownload id={item.presentation.$id} className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-fuchsia-950">{item.presentation.title}</span><span className="text-xs text-fuchsia-800">{item.presentation.watchedAt ? 'Viewed' : 'Posted'} · {formatDate(item.date)}</span></PresentationDownload>:<a href={item.presentation.url || PRESENTATION_FOLDER_URL} target="_blank" rel="noreferrer" className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-fuchsia-950">{item.presentation.title}</span><span className="text-xs text-fuchsia-800">{item.presentation.watchedAt ? 'Viewed' : 'Posted'} · {formatDate(item.date)}</span></a>}{isOwner && <button className="text-xs font-semibold text-red-700 underline" onClick={() => window.confirm('Delete this link?') && void deletePresentationLink(item.presentation.$id)}>Delete</button>}</div>)}</CompactWeekSection>}
...
function TextMaterialLink({ item }: { item: Extract<WeeklyMaterial, { kind: 'text' }> }) {
  const content = <><span className="block truncate text-sm font-semibold text-emerald-950">{item.text.title}</span><TextPurposeLabels value={item.assignment}/><span className="text-xs text-emerald-800">{formatDate(item.date)}{item.text.author ? ` · ${item.text.author}` : ''}</span></>;
...
function WeeklyClassReview({ materials, isOwner }: { materials: WeeklyMaterial[]; isOwner: boolean }) {
  ...
  return (
    ...
    <section>
      ...
      {weeks.length === 0 ? (
        <Card><p className="text-sm text-gray-500">Notes, writing prompts, quizzes, discussions, texts, and links will appear here by week.</p></Card>
      ) : (
            ...
            return (
              <div key={week} className="overflow-hidden rounded-xl border border-gray-200 bg-white">
                <button className="flex w-full items-center justify-between gap-3 p-4 text-left hover:bg-gray-50" onClick={() => setOpenWeeks(current => { const next = new Set(current); if (next.has(week)) next.delete(week); else next.add(week); return next; })}>
                  ...
                  <span className="text-xs text-gray-500">{counts.notes} notes · {counts.writingPrompts} writing prompts · {counts.discussions} discussions · {counts.texts} texts · {counts.quizzes} quizzes · {counts.presentations} links</span>
                </button>
                {isOpen && (
                  <div className="space-y-3 border-t bg-gray-50 p-4">
                    ...
                    {counts.presentations > 0 && <CompactWeekSection title="Links" count={counts.presentations} color="fuchsia" open={openSections.has(`${week}-presentations`)} onToggle={() => setOpenSections(current => toggleSetValue(current, `${week}-presentations`))}>{items.filter((item): item is Extract<WeeklyMaterial,{kind:'presentation'}> => item.kind === 'presentation').map(item => <div key={item.presentation.$id} className="flex items-center gap-3 border-t border-fuchsia-100 px-4 py-2.5"><input aria-label={`Mark ${item.presentation.title} viewed`} type="checkbox" className="h-5 w-5" checked={Boolean(item.presentation.watchedAt)} disabled={!isOwner} onChange={event=>void setPresentationWatched(item.presentation.$id,event.target.checked)}/><a href={item.presentation.url} target="_blank" rel="noreferrer" className="min-w-0 flex-1"><span className="block truncate text-sm font-semibold text-fuchsia-950">{item.presentation.title}</span><span className="text-xs text-fuchsia-800">{item.presentation.watchedAt?'Viewed':'Posted'} · {formatDate(item.date)}</span></a></div>)}</CompactWeekSection>}
