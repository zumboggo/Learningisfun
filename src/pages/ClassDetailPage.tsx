...
function AssignTextsToClassModal({ open, classId, teacherId, onClose }: { open: boolean; classId: string; teacherId: string; onClose: () => void }) {
  ...
  const createAndAssign = async () => {
    setBusy(true); setError('');
    try {
      await createText({ teacherId, title: title.trim(), author: author.trim(), source: source.trim(), paragraphs: contentMode === 'full' ? splitParagraphs(copiedText) : [], classIds: [classId], originalPdf: contentMode === 'full' ? originalPdf : undefined, contentMode, externalUrl: externalUrl.trim(), schedule });
      onClose();
