...
function CreateDiscussionModal({teacherId,onClose}:{teacherId:string;onClose:()=>void}){const[type,setType]=useState<'text'|'question'|'qft'>('text');const[classId,setClassId]=useState('');const[textId,setTextId]=useState('');const[title,setTitle]=useState('');const[prompt,setPrompt]=useState('');const classes=useLiveQuery(()=>db.classes.where('teacherId').equals(teacherId).toArray(),[teacherId]);const selectedClassId=classId||classes?.[0]?.$id||'';const texts=useLiveQuery(async()=>{if(!selectedClassId)return [];const ids=(await db.text_assignments.where('classId').equals(selectedClassId).toArray()).map(a=>a.textId);return ids.length?db.texts.where('$id').anyOf(ids).toArray():[]},[selectedClassId]);return <Modal open onClose={onClose} title="Start discussion"><div className="space-y-4"><div className="grid grid-cols-3 gap-2">{(['text','question','qft'] as const).map(x=><Button key={x} size="sm" variant={type===x?'primary':'secondary'} onClick={()=>setType(x)}>{x==='qft'?'QFT prompt':x==='text'?'Uploaded text':'Question'}</Button>)}</div><select className={input} value={selectedClassId} onChange={e=>setClassId(e.target.value)}>{classes?.map(c=><option key={c.$id} value={c.$id}>{classLabel(c)}</option>)}</select>{type==='text'&&<select className={input} value={textId} onChange={e=>setTextId(e.target.value)}><option value="">Choose a text</option>{texts?.map(t=><option key={t.$id} value={t.$id}>{t.title}</option>)}</select>}<input className={input} value={title} onChange={e=>setTitle(e.target.value)} placeholder="Discussion title"/>{type!=='text'&&<textarea className={input} rows={4} value={prompt} onChange={e=>setPrompt(e.target.value)} placeholder={type==='qft'?'Question Focus prompt':'Question for the class'}/>}<Button disabled={!selectedClassId||!title.trim()||(type==='text'&&!textId)} onClick={()=>void createClassSession(selectedClassId,teacherId,{title,discussionType:type,textId:type==='text'?textId:null,promptMarkdown:prompt}).then(onClose)}>Create discussion</Button></div></Modal>}
