...
import { createText, paragraphsFromFile, setTextClasses, splitParagraphs } from '@/services/text.service';
import type { Class, LearningText } from '@/types';

export function TextsPage() {
  const { user, isTeacher } = useAuth(); const [creating, setCreating] = useState(false); const [assigning, setAssigning] = useState<LearningText | null>(null);
  ...
  return <div className="p-4 max-w-4xl mx-auto space-y-5"><header className="flex items-center justify-between"><div><h1 className="text-2xl font-bold">Texts</h1><p className="text-sm text-gray-500">Read closely, then bring the text into Discussions.</p></div>{isTeacher && <Button onClick={() => setCreating(true)}>Upload text</Button>}</header>
    {rows?.length ? <TextsByWeek rows={rows} classes={classes||[]} isTeacher={isTeacher} onAssign={setAssigning}/> : <EmptyState title="No texts yet" message={isTeacher ? 'Upload a text for close reading and discussion.' : 'Assigned readings will appear here.'}/>} 
...
function TextsByWeek({rows,classes,isTeacher,onAssign}:{rows:Array<{text:LearningText;assignments:Array<{classId:string;assignedAt:string}>}>;classes:Array<Class|undefined>;isTeacher:boolean;onAssign:(text:LearningText)=>void}) {
  ...
  const weeks=[...groups.entries()].sort((a,b)=>b[0].localeCompare(a[0])); const [open,setOpen]=useState(weeks[0]?.[0]||'');
  return <div className="space-y-3">{weeks.map(([week,items])=><div key={week}><button className="w-full rounded-lg border bg-white p-3 text-left font-semibold" onClick={()=>setOpen(open===week?'':week)}>{open===week?'▾':'▸'} Week of {new Date(`${week}T00:00:00`).toLocaleDateString(undefined,{month:'short',day:'numeric',year:'numeric'})} <span className="text-sm font-normal text-gray-500">({items.length})</span></button>{open===week&&<div className="mt-2 space-y-2">{items.map(({text,assignments})=><Card key={text.$id}><div className="flex justify-between gap-3"><Link to={`/texts/${text.$id}`}><h2 className="font-semibold">{text.title}</h2><p className="text-sm text-gray-500">{text.author||'Unknown author'} · {assignments.map(a=>classLabel(classes.find(c=>c?.$id===a.classId))).join(', ')||'Not assigned'}</p></Link>{isTeacher&&<Button size="sm" variant="secondary" onClick={()=>onAssign(text)}>Assign</Button>}</div></Card>)}</div>}</div>)}</div>;
}
