...
function CreateTextModal({teacherId,classes,onClose}:{teacherId:string;classes:Array<{id:string;name:string}>;onClose:()=>void}) {
  ...
  return <Modal open onClose={()=>!busy&&onClose()} title="Add text"><div className="space-y-3 max-h-[75vh] overflow-auto">{saveError&&<p role="alert" className="text-sm text-red-700">{saveError}</p>}<div className="grid grid-cols-2 gap-2 rounded-lg bg-gray-100 p-1"><button className={`rounded-md px-3 py-2 text-sm font-medium ${mode==='full'?'bg-white shadow-sm':''}`} onClick={()=>setMode('full')}>Full text</button><button className={`rounded-md px-3 py-2 text-sm font-medium ${mode==='link'?'bg-white shadow-sm':''}`} onClick={()=>setMode('link')}>Link only</button></div><input className={input} placeholder="Title" value={title} onChange={e=>setTitle(e.target.value)}/><input className={input} placeholder="Author" value={author} onChange={e=>setAuthor(e.target.value)}/><div className="space-y-3">{classes.map(c=><div key={c.id} className="flex flex-wrap items-start justify-between gap-2 rounded-lg border p-3"><label className="flex gap-2 text-sm"><input type="checkbox" checked={selected.has(c.id)} onChange={()=>setSelected(current=>{const next=new Set(current);if(next.has(c.id))next.delete(c.id);else next.add(c.id);return next})}/>{c.name}</label><ClassReadingDate purpose={purposes[c.id]} onPurposeChange={value=>setPurposes(current=>({...current,[c.id]:value}))} name={c.name} value={dates[c.id]||''} onChange={value=>setDates(current=>({...current,[c.id]:value}))} disabled={!selected.has(c.id)}/></div>)}</div>{mode==='link'?<label className="block text-sm font-medium">Source (optional link)<input className={input} type="url" placeholder="https://…" value={externalUrl} onChange={e=>setExternalUrl(e.target.value)}/></label>:<><label className="block text-sm font-medium">Source (optional link)<input type="url" className={input} placeholder="https://…" value={externalUrl} onChange={e=>setExternalUrl(e.target.value)}/></label><TextFileUpload onBusyChange={setBusy} onImport={(content,file)=>{setRaw(content);setParagraphs([]);setOriginalPdf(file)}}/><MarkdownPasteEditor value={raw} onChange={value=>{setRaw(value);setParagraphs([])}} rows={10}/><details><summary className="cursor-pointer text-sm font-medium">Paragraph preview ({preview.length})</summary><div className="mt-2 space-y-2">{preview.map((p,i)=><div key={i} className="rounded-lg border bg-white p-3"><p className="mb-2 text-xs text-gray-400">Paragraph {i+1}</p><Markdown content={p}/></div>)}</div></details></>}<Button loading={busy} disabled={!valid} onClick={()=>{setBusy(true);void createText({teacherId,title:title.trim(),author:author.trim(),source:'',paragraphs:mode==='full'?preview:[],originalPdf:mode==='full'?originalPdf:undefined,classIds:[...selected],classDates:dates,classPurposes:purposes,contentMode:mode,externalUrl:externalUrl.trim()}).then(onClose).catch(cause=>setSaveError(cause instanceof Error?cause.message:'Could not save the text.')).finally(()=>setBusy(false))}}>Save text</Button></div></Modal>;
}
