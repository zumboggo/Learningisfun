...
import { createText, paragraphsFromFile, splitParagraphs } from '@/services/text.service';
...
function CreateTextModal({teacherId,classes,onClose}:{teacherId:string;classes:Array<{id:string;name:string}>;onClose:()=>void}) {
  ...
  const valid = title.trim() && (mode==='link' ? /^https?:\/\//i.test(externalUrl) : preview.length > 0);
  return <Modal open onClose={onClose} title="Add text"><div className="space-y-3 max-h-[75vh] overflow-auto"><div className="grid grid-cols-2 gap-2 rounded-lg bg-gray-100 p-1"><button className={`rounded-md px-3 py-2 text-sm font-medium ${mode==='full'?'bg-white shadow-sm':''}`} onClick={()=>setMode('full')}>Full text</button><button className={`rounded-md px-3 py-2 text-sm font-medium ${mode==='link'?'bg-white shadow-sm':''}`} onClick={()=>setMode('link')}>Link only</button></div><input className={input} placeholder="Title" value={title} onChange={e=>setTitle(e.target.value)}/><input className={input} placeholder="Author" value={author} onChange={e=>setAuthor(e.target.value)}/><input className={input} placeholder="Source (optional)" value={source} onChange={e=>setSource(e.target.value)}/>{mode==='link'?<input className={input} type="url" placeholder="https://…" value={externalUrl} onChange={e=>setExternalUrl(e.target.value)}/>:<><input type="file" accept=".txt,.docx,text/plain" onChange={e=>{const f=e.target.files?.[0];if(f)void paragraphsFromFile(f).then(rows=>{setParagraphs(rows);setRaw(rows.join('\n\n'))})}}/><MarkdownPasteEditor value={raw} onChange={value=>{setRaw(value);setParagraphs([])}} rows={10}/><div><p className="text-sm font-medium">Paragraph preview ({preview.length})</p><div className="mt-2 space-y-2">{preview.map((p,i)=><div key={i} className="rounded-lg border bg-white p-3"><p className="mb-2 text-xs text-gray-400">Paragraph {i+1}</p><Markdown content={p}/></div>)}</div></div></>}<ClassChecks classes={classes} selected={selected} onChange={setSelected}/><Button loading={busy} disabled={!valid} onClick={()=>{setBusy(true);void createText({teacherId,title:title.trim(),author:author.trim(),source:source.trim(),paragraphs:mode==='full'?preview:[],classIds:[...selected],contentMode:mode,externalUrl:mode==='link'?externalUrl.trim():''}).then(onClose).finally(()=>setBusy(false))}}>Save text</Button></div></Modal>;
}
