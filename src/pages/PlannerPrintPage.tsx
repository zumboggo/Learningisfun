...
export const printExcerpt = (value: string) => {
...
function ActivityRow({ label, children }: { label: 'I' | 'We' | 'They' | 'Check'; children: string }) {
  return <div className={`grid grid-cols-[2.5rem_1fr] gap-1 rounded-md px-1.5 py-1 ${activityTone[label]}`}><b><PrintCheck/>{label}</b><span>{printExcerpt(children) || '—'}</span></div>;
}
...
export function PlannerPrintSheet({data}: {data: WeeklyPlanData}) {
  // Each section gets a half-page budget; keep Blue and Red adjacent.
  const ordered = [...data.week.blocks].sort((a, b) => {
    const rank = (code: string) => code === 'WL-B' ? 0 : code === 'WL-R' ? 1 : 2;
    return rank(a.code) - rank(b.code);
  });
  const groups = ordered.map(block => [block]);
  ...
  return <main className="planner-print mx-auto max-w-[900px] bg-white p-5 text-gray-950">
    ...
    <section className="planner-course-grid mt-2 space-y-2">{groups.map(blocks => {
