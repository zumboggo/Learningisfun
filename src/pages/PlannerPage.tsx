...
import { addWeeklyLessonDefaults } from '@/services/unit-planning';
...
export function PlannerPage(){
 ...
 return <div className="mx-auto max-w-[1600px] space-y-3 p-3 sm:p-4">
  ...
  {parsed&&data&&<>
   ...
   {active&&user&&data.week.key===selectedWeek&&<WeeklyPlannerEditor key={active.$id+selectedWeek} initial={data} suggestedWeek={parsed.weeks.find(week=>week.key===selectedWeek)} record={existing} sourceId={active.$id} userId={user.$id} units={units} onUnitsChange={setUnits} onSaved={record=>setPlans(old=>[record,...old.filter(item=>item.$id!==record.$id&&item.weekKey!==record.weekKey)])}/>}
