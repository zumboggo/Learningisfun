...
export function PlannerPage(){
 ...
 return <div className="mx-auto max-w-6xl space-y-5 p-4 sm:p-6">
  <header className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-blue-600">Teacher only</p><h1 className="text-3xl font-bold text-gray-950">Weekly Planning</h1><p className="mt-1 text-gray-500">Your unit bank supplies the resources. Arrange and adjust each class period.</p></div>{active&&<label className="cursor-pointer rounded-lg border bg-white px-4 py-2 text-sm font-semibold">Re-import annual plan<input className="hidden" type="file" accept=".txt,text/plain" onChange={event=>void pickFile(event.target.files?.[0])}/></label>}</header>
