    ...
    {text.contentMode !== 'link' && (readingVersion === 'original' || activeVersion?.status === 'ready') && <div className="space-y-5">
      ...
      <aside className="space-y-3"><details className="rounded-xl border p-3"><summary className="cursor-pointer text-sm font-medium">Annotation search and filters · {visible.length} notes</summary><Card className="space-y-3"><div className="flex items-center justify-between"><h2 className="font-semibold">Annotations</h2><span className="text-xs text-gray-500">{visible.length} shown</span></div><input aria-label="Search annotations" className="w-full rounded-lg border px-3 py-2 text-sm" placeholder="Search notes or tags" value={query} onChange={event=>setQuery(event.target.value)} /><div className="grid grid-cols-2 gap-2"><select className="rounded-lg border px-2 py-2 text-sm" value={filter} onChange={event=>setFilter(event.target.value as typeof filter)}><option value="all">All types</option><option value="question">Questions</option>{tqe?<><option value="thought">Thoughts</option><option value="epiphany">Epiphanies</option></>:<option value="observation">Observations</option>}<option value="page_note">Page notes</option><option value="mine">Mine</option></select><select className="rounded-lg border px-2 py-2 text-sm" value={sort} onChange={event=>setSort(event.target.value as typeof sort)}><option value="location">Text order</option><option value="newest">Newest</option><option value="oldest">Oldest</option></select></div></Card></details>
        ...
        {!visible.length && <Card><p className="text-sm text-gray-500">No annotations match these filters.</p></Card>}
            ...
            <main aria-label="Reading text" style={{ fontSize }} className="space-y-8 rounded-2xl bg-[#faf9f6] p-5 text-slate-900 sm:p-8">{paragraphs?.map((paragraph,index) => { const adapted=adaptedByOriginal.get(paragraph.$id); const annotate=(selection:string)=>startComposer({paragraphId:paragraph.$id,selectedText:selection,kind:'annotation'}); const highlight=(selection:string)=>void makeHighlight(paragraph.$id,selection); const passageNotes = visible.filter(a => a.paragraphId === paragraph.$id); const marks = passageNotes.map(a => ({id:a.$id,quote:a.selectedText || ''})); const shown=readingVersion==='original'||!adapted?paragraph:{...paragraph,content:adapted.content}; return <div key={paragraph.$id} className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_320px]"><div className="reader-prose mx-auto w-full max-w-[65ch]">{readingVersion!=='original'&&compare&&adapted&&<ParagraphCard article paragraph={paragraph} index={index} label="Original" readOnly={Boolean(isParent)} onAnnotate={annotate} onHighlight={highlight} highlights={marks} onOpen={()=>setActiveParagraph(paragraph.$id)}/>}<ParagraphCard article highlights={marks} onOpen={()=>setActiveParagraph(paragraph.$id)} key={paragraph.$id} paragraph={shown} index={index} label={readingVersion==='original'?undefined:supportLevelLabel(readingVersion)} readOnly={Boolean(isParent)} onAnnotate={annotate} onHighlight={highlight} density={tqe ? (annotations || []).filter(a=>a.paragraphId===paragraph.$id && a.tqeType && (a.kind||'annotation')==='annotation' && a.visibility!=='private' && a.moderationStatus==='visible').length : undefined}/>{passageNotes.length > 0 && <button className="mt-2 font-sans text-xs text-amber-800" aria-expanded={activeParagraph===paragraph.$id} onClick={()=>setActiveParagraph(activeParagraph===paragraph.$id?null:paragraph.$id)}>{passageNotes.length} passage notes</button>}</div>{activeParagraph===paragraph.$id && <aside aria-label={`Notes for paragraph ${index+1}`} className="space-y-3 font-sans text-base"><button className="text-sm text-slate-600" onClick={()=>setActiveParagraph(null)}>Close notes ×</button>{composer?.paragraphId===paragraph.$id && composerView}{passageNotes.map(renderAnnotation)}</aside>}</div>})}</main>
    </div>}
