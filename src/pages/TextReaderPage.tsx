import { publicReadingRequest, type PublicReading } from '@/services/public-reading.service';
...
export function TextReaderPage({publicReading}:{publicReading?:PublicReading}={}) {
  const { textId }=useParams(),[search,setSearch]=useSearchParams();
  const {user,isTeacher:teacher,isParent:parent}=useAuth();
  const isTeacher=teacher&&!publicReading,isParent=parent||Boolean(publicReading);
  const shell=useRef<HTMLDivElement>(null);
  const [fullscreen,setFullscreen]=useState(false),[fullscreenError,setFullscreenError]=useState('');
  useEffect(()=>{const changed=()=>setFullscreen(document.fullscreenElement===shell.current);document.addEventListener('fullscreenchange',changed);return()=>document.removeEventListener('fullscreenchange',changed);},[]);
  ...
  const cachedText=useLiveQuery(()=>textId&&!publicReading?db.texts.get(textId):undefined,[textId,publicReading]);
  const text=publicReading?.text||cachedText;
  const cachedParagraphs=useLiveQuery(()=>textId&&!publicReading?db.text_paragraphs.where('textId').equals(textId).sortBy('sortOrder'):[],[textId,publicReading]);
  const paragraphs=publicReading?.paragraphs||cachedParagraphs;
  ...
  const availableClasses=useLiveQuery(async()=>{
    if(!textId||!user||publicReading)return [];
  ...
  useEffect(()=>{
    if(publicReading||!textId||!(classId||requestedClass))return;
  ...
  },[textId,classId,requestedClass,publicReading]);
  ...
  if(!text)return <TextPublicReaderPage/>;
  if(!user&&!publicReading)return <div className="p-6">Loading…</div>;
  if(!publicReading&&!isTeacher&&(!classId||text.status!=='published'))return <TextPublicReaderPage/>;
  ...
  return <div ref={shell} className="reader-shell mx-auto max-w-6xl space-y-5 overflow-auto bg-white p-4 sm:p-6">
    <header className="space-y-3"><div className="flex flex-wrap items-center gap-3"><Link to={isTeacher?'/classes':'/dashboard'} className="inline-flex min-h-11 items-center rounded-xl border px-4">Home</Link><Link to="/texts" className="text-sm text-blue-700">← Texts</Link>{classId&&readingAssignment?.isAssignedReading&&<Link to={`/discussions/texts/${textId}/${classId}`} className="ml-auto inline-flex min-h-11 items-center rounded-xl bg-blue-50 px-4 font-semibold text-blue-800">Discuss text →</Link>}</div><h1 className="reader-title text-3xl font-semibold sm:text-4xl">{text.title}</h1><p className="text-slate-500">{text.author}</p>{readingAssignment&&<TextPurposeLabels value={readingAssignment}/>}<TextSource source={text.source} url={text.contentMode!=='link'?text.externalUrl:undefined}/>{isTeacher&&<CopyTextLinkButton textId={text.$id} title={text.title}/>}</header>
    <div className="flex justify-end"><button className="min-h-11 rounded-xl border px-3 text-sm" onClick={()=>{setFullscreenError('');const operation=document.fullscreenElement?document.exitFullscreen():shell.current?.requestFullscreen?.();if(!operation){setFullscreenError('Fullscreen is not supported by this browser.');return;}void operation.catch(()=>setFullscreenError('Fullscreen was blocked by this browser.'));}}>{fullscreen?'Exit fullscreen':'Fullscreen'}</button></div>{fullscreenError&&<p role="status">{fullscreenError}</p>}
    ...
    {mode==='original'&&(text.originalPdfId?<OriginalPdf textId={text.$id} embedded publicAccess={Boolean(publicReading)}/>:<Card><p>No original uploaded file was retained for this text. Article Mode shows the saved reading.</p>{text.externalUrl&&/^https?:\/\//i.test(text.externalUrl)&&<a className="text-blue-700 underline" href={text.externalUrl} target="_blank" rel="noreferrer">Open original source ↗</a>}</Card>)}
...
export function TextPublicReaderPage() {
  const {textId=''}=useParams();
  return <PublicReaderSession key={textId} textId={textId}/>;
}
function PublicReaderSession({textId}:{textId:string}) {
  const [data,setData]=useState<PublicReading>(),[error,setError]=useState('');
  useEffect(()=>{let active=true;void publicReadingRequest<PublicReading>(textId).then(result=>{if(active)setData(result);}).catch(cause=>{if(active)setError(cause.message);});return()=>{active=false;};},[textId]);
  if(data)return <TextReaderPage publicReading={data}/>;
  return <div className="mx-auto max-w-xl space-y-4 p-6"><h1 className="text-2xl font-semibold">Shared reading</h1><p role={error?'alert':'status'}>{error||'Loading shared text…'}</p>{error&&<Link className="inline-block rounded-lg bg-blue-700 px-4 py-3 text-white" to="/login" state={{from:`/texts/${textId}`}}>Sign in to your class</Link>}</div>;
}
