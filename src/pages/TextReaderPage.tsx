...
export function TextReaderPage() {
  ...
  const [readerMode,setReaderMode]=useState<'article'|'original'|'annotate'>('article');
  ...
  const startComposer = (state: ComposerState) => { setComposer(state); setActiveParagraph(state.paragraphId); setDraft(''); setTags(''); setType(tqe ? 'thought' : 'observation'); setPrivateNote(false); };
  const post = async () => {
  ...
  return <div className="reader-shell mx-auto max-w-6xl space-y-5 p-4 sm:p-6">
    <header><div className="flex items-center gap-3"><Link to={isTeacher?"/classes":"/dashboard"} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 font-semibold text-slate-700 hover:bg-slate-50"><span aria-hidden="true">⌂</span> Home</Link><Link to="/texts" className="text-sm text-blue-600">← Texts</Link></div><div className="mt-2 flex flex-wrap items-start justify-between gap-3"><div><h1 className="reader-title text-2xl font-semibold sm:text-4xl">{text.title}</h1><p className="text-gray-500">{text.author}</p>{readingAssignment&&<TextPurposeLabels value={readingAssignment}/>}<TextSource source={text.source} url={text.contentMode!=='link'?text.externalUrl:undefined}/></div>{isTeacher&&<CopyTextLinkButton textId={text.$id} title={text.title}/>} </div>{text.contentMode !== 'link' && !isTeacher && !isParent && !tqe && sharedMine < ANNOTATIONS_TO_UNLOCK && <p className="mt-2 rounded bg-blue-50 p-2 text-sm">Add {ANNOTATIONS_TO_UNLOCK-sharedMine} more shared observation{ANNOTATIONS_TO_UNLOCK-sharedMine===1?'':'s'} or question{ANNOTATIONS_TO_UNLOCK-sharedMine===1?'':'s'} to reveal classmates’ notes. Private highlights do not count.</p>}</header>

    <ReaderToolbar mode={readerMode} panel={readerPanel} size={fontSize} onSize={setFontSize} onArticle={()=>{setReaderMode('article');setReaderPanel(null);setReadingVersion('original');setActiveParagraph(null);}} onOriginal={()=>{setReaderMode('original');setReaderPanel(null);}} onPanel={panel=>{setReaderPanel(panel);if(panel==='tqe'||panel==='notes')setReaderMode('annotate');else if(readerMode==='original')setReaderMode('article');}}>
      {readerPanel==='tqe'&&<>{isTeacher && text.teacherId === user.$id && <label className="block text-sm font-medium">Annotation style<select className="ml-3 rounded-lg border p-2" value={tqe?(thoughtOnly?'thought':'tqe'):'regular'} onChange={event => void setAnnotationMode(text.$id,event.target.value==='regular'?'regular':'tqe',event.target.value==='thought'?'thought':'full').catch(cause => setAnnotationError(cause.message))}><option value="tqe">TQE · Thoughts, Questions, Epiphanies</option><option value="thought">TQE introduction · Thought only</option><option value="regular">Regular annotations</option></select></label>}{tqe && !isParent && text.contentMode !== 'link' && <div className="rounded-xl border bg-slate-50 p-4"><p className="font-semibold">Thought {progress.thought?'✓':'○'}{!thoughtOnly && <> · Question {progress.question?'✓':'○'} · Epiphany {progress.epiphany?'✓':'○'}</>}</p><p className="mt-1 text-sm text-slate-600">{thoughtOnly?'One shared Thought reveals classmates’ notes. Notice a word or choice in the writing, then explain what it does.':'One of each reveals classmates’ notes. Select a passage, add your note, then choose one to Bring it.'}</p><details className="mt-2 text-sm"><summary className="cursor-pointer font-medium">What makes a useful TQE?</summary><p className="mt-2">Thought: name something you noticed in the writing. Question: name something you cannot work out. Epiphany: explain a connection or how your reading changed.</p><p className="mt-2">Ask why the writer made that choice. For a translation, ask why the translator chose that word.</p></details></div>}{tqe && classId && !isParent && text.contentMode !== 'link' && <details className="rounded-xl border p-3"><summary className="cursor-pointer text-sm font-medium">TQE workspace</summary><TqeWorkspace thoughtOnly={thoughtOnly} key={text.$id+classId} textId={text.$id} classId={classId} userId={user.$id} teacher={Boolean(isTeacher)} annotations={annotations || []} /></details>}{!tqe&&<p className="text-sm text-slate-600">This text uses regular annotations. Use a pencil beside a paragraph to add a note.</p>}</>}
      {readerPanel==='support'&&<>{text.contentMode !== 'link' && Boolean(paragraphs?.length) && <details className="rounded-xl border p-3"><summary className="cursor-pointer text-sm font-medium">Reading support · {readingVersion==='original'?'Original':supportLevelLabel(readingVersion)}</summary><ReadingSupportToolbar readingVersion={readingVersion} setReadingVersion={setReadingVersion} compare={compare} setCompare={setCompare} versions={versions||[]} /></details>}<details className="rounded-xl border p-3"><summary className="cursor-pointer text-sm font-medium">Read aloud and fullscreen</summary><ReadingTools key={`${text.$id}:${readingVersion}`} title={text.title} paragraphs={text.contentMode === 'link' || (readingVersion !== 'original' && activeVersion?.status !== 'ready') ? [] : (paragraphs || []).map(paragraph => readingVersion === 'original' ? paragraph.content : (adaptedByOriginal.get(paragraph.$id)?.content || paragraph.content))} unavailable={text.contentMode === 'link' ? 'Open the linked reading to use its browser reading tools. The app cannot read text from another website directly.' : text.originalPdfId && !paragraphs?.length ? 'This PDF has no extracted text. Open the original PDF to use its viewer’s reading tools; scans need OCR before they can be read aloud.' : 'Choose Original or wait for the selected reading version to be ready.'}/></details></>}
      {readerPanel==='notes'&&<>{!isParent && text.contentMode !== 'link' && <Button size="sm" variant="secondary" onClick={() => startComposer({paragraphId:'page',selectedText:'',kind:'page_note'})}>+ Page note</Button>}      <aside className="space-y-3"><details className="rounded-xl border p-3"><summary className="cursor-pointer text-sm font-medium">Annotation search and filters · {visible.length} notes</summary><Card className="space-y-3"><div className="flex items-center justify-between"><h2 className="font-semibold">Annotations</h2><span className="text-xs text-gray-500">{visible.length} shown</span></div><input aria-label="Search annotations" className="w-full rounded-lg border px-3 py-2 text-sm" placeholder="Search notes or tags" value={query} onChange={event=>setQuery(event.target.value)} /><div className="grid grid-cols-2 gap-2"><select className="rounded-lg border px-2 py-2 text-sm" value={filter} onChange={event=>setFilter(event.target.value as typeof filter)}><option value="all">All types</option><option value="question">Questions</option>{tqe?<><option value="thought">Thoughts</option><option value="epiphany">Epiphanies</option></>:<option value="observation">Observations</option>}<option value="page_note">Page notes</option><option value="mine">Mine</option></select><select className="rounded-lg border px-2 py-2 text-sm" value={sort} onChange={event=>setSort(event.target.value as typeof sort)}><option value="location">Text order</option><option value="newest">Newest</option><option value="oldest">Oldest</option></select></div></Card></details>
        {visible.filter(annotation=>annotation.paragraphId==='page').map(renderAnnotation)}
        {!visible.length && <Card><p className="text-sm text-gray-500">No annotations match these filters.</p></Card>}
      </aside></>}
      {readerPanel==='more'&&<>{availableClasses && availableClasses.length > 1 && <label className="block text-sm font-medium">Class section<select className="ml-3 rounded-lg border p-2" value={classId || ''} onChange={event => { setSelectedClass(event.target.value); setComposer(null); }}>{availableClasses.map(c => c && <option key={c.$id} value={c.$id}>{c.courseName} · {c.name}</option>)}</select></label>}<TextViewControls size={fontSize} onSize={setFontSize} title={readingVersion === 'original' ? text.title : `${text.title} — ${supportLevelLabel(readingVersion)}`} paragraphs={text.contentMode === 'link' || (readingVersion !== 'original' && activeVersion?.status !== 'ready') ? [] : (paragraphs || []).map(p => readingVersion === 'original' ? p.content : (adaptedByOriginal.get(p.$id)?.content || p.content))}/></>}
    </ReaderToolbar>
    {readerMode==='original'&&(text.originalPdfId?<OriginalPdf key={text.$id} textId={text.$id} embedded/>:<Card><p className="text-sm text-slate-600">No original uploaded file was retained for this text. Article Mode shows the saved text; older Word uploads need to be re-uploaded as a PDF to preserve their original layout.</p>{text.externalUrl&&/^https?:\/\//i.test(text.externalUrl)&&<a href={text.externalUrl} target="_blank" rel="noreferrer" className="mt-3 inline-block text-blue-700 underline">Open original source ↗</a>}</Card>)}
    ...
    {readerMode!=='original' && text.contentMode !== 'link' && readingVersion !== 'original' && activeVersion?.status !== 'ready' && <Card className="space-y-3 border-blue-200 bg-blue-50"><h2 className="font-semibold text-blue-950">{supportLevelLabel(readingVersion)} version</h2><p className="text-sm text-blue-900">{activeVersion?.status==='generating'?'Someone has already requested this shared version. It is still being prepared.':activeVersion?.status==='failed'?'The previous attempt did not finish. You can try again.':'This shared reading-support version has not been created yet. The first request uses AI; afterward everyone assigned this text reuses the saved version.'}</p>{supportError&&<p className="rounded-lg bg-red-50 p-3 text-sm text-red-700">{supportError}</p>}{!isParent&&activeVersion?.status!=='generating'&&<Button loading={generatingVersion===readingVersion} onClick={()=>void requestSupport(readingVersion)}>{activeVersion?.status==='failed'?'Try again':'Create shared version'}</Button>}{activeVersion?.status==='generating'&&<Button variant="secondary" onClick={()=>classId&&void syncTextFromServer(text.$id,classId)}>Check again</Button>}</Card>}
    {readerMode!=='original' && text.contentMode !== 'link' && (readingVersion === 'original' || activeVersion?.status === 'ready') && <div className="space-y-5">


            <main aria-label="Reading text" style={{ fontSize }} className="reader-paper space-y-8 rounded-2xl bg-white px-8 py-6 text-slate-900 sm:px-12 sm:py-8">{paragraphs?.map((paragraph,index) => { const adapted=adaptedByOriginal.get(paragraph.$id); const annotate=(selection:string)=>startComposer({paragraphId:paragraph.$id,selectedText:selection,kind:'annotation'}); const highlight=(selection:string)=>void makeHighlight(paragraph.$id,selection); const passageNotes = visible.filter(a => a.paragraphId === paragraph.$id); const marks = passageNotes.map(a => ({id:a.$id,quote:a.selectedText || ''})); const shown=readingVersion==='original'||!adapted?paragraph:{...paragraph,content:adapted.content}; return <div key={paragraph.$id} className={readerMode==='annotate'&&activeParagraph===paragraph.$id?"grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_300px]":"mx-auto max-w-[70ch]"}><div className="reader-prose mx-auto w-full max-w-[65ch]">{readingVersion!=='original'&&compare&&adapted&&<ParagraphCard article paragraph={paragraph} index={index} label="Original" readOnly={Boolean(isParent)||readerMode!=='annotate'} onAnnotate={annotate} onHighlight={highlight} highlights={readerMode==='annotate'?marks:[]} onOpen={()=>setActiveParagraph(paragraph.$id)}/>}<ParagraphCard article highlights={readerMode==='annotate'?marks:[]} onOpen={()=>setActiveParagraph(paragraph.$id)} key={paragraph.$id} paragraph={shown} index={index} label={readingVersion==='original'?undefined:supportLevelLabel(readingVersion)} readOnly={Boolean(isParent)||readerMode!=='annotate'} onAnnotate={annotate} onHighlight={highlight} density={readerMode==='annotate' && tqe ? (annotations || []).filter(a=>a.paragraphId===paragraph.$id && a.tqeType && (a.kind||'annotation')==='annotation' && a.visibility!=='private' && a.moderationStatus==='visible').length : undefined}/>{readerMode==='annotate' && passageNotes.length > 0 && <button className="mt-2 font-sans text-xs text-amber-800" aria-expanded={activeParagraph===paragraph.$id} onClick={()=>setActiveParagraph(activeParagraph===paragraph.$id?null:paragraph.$id)}>{passageNotes.length} passage notes</button>}</div>{readerMode==='annotate' && activeParagraph===paragraph.$id && <aside aria-label={`Notes for paragraph ${index+1}`} className="space-y-3 font-sans text-base"><button className="text-sm text-slate-600" onClick={()=>setActiveParagraph(null)}>Close notes ×</button>{composer?.paragraphId===paragraph.$id && composerView}{passageNotes.map(renderAnnotation)}</aside>}</div>})}</main>
    </div>}
    <Link to="/discussions" className="inline-block font-medium text-blue-600">Continue in Discussions →</Link>
    {composer?.paragraphId === 'page' && composerView}
...
export function ParagraphCard({ highlights, onOpen, article, paragraph, index, label, readOnly, onAnnotate, onHighlight, density }: { highlights?: Array<{id:string;quote:string}>; onOpen?:()=>void; article:boolean; density?:number; paragraph:TextParagraph;index:number;label?:string;readOnly:boolean;onAnnotate:(selection:string)=>void;onHighlight:(selection:string)=>void }) {
  const [selection,setSelection]=useState('');
  const passageRef = useRef<HTMLDivElement>(null);
  const capture = () => {
    const chosen = window.getSelection();
    setSelection(chosen?.anchorNode && chosen.focusNode && passageRef.current?.contains(chosen.anchorNode) && passageRef.current.contains(chosen.focusNode) ? chosen.toString().trim().slice(0,2000) : '');
  };
  ...
  return <section aria-label={`Paragraph ${index+1}`} className={article ? 'relative space-y-2' : 'space-y-3 rounded-xl border bg-white p-4 text-slate-900'}>
    {Boolean(!article || label || density) && <p className="text-xs font-medium text-gray-500">{label? `${label} · `:''}Paragraph {index+1}{density !== undefined && <span className="ml-2 text-blue-700">· {density} TQE notes</span>}</p>}
    <div ref={passageRef} onMouseUp={capture} onTouchEnd={capture} onKeyUp={capture}><Markdown content={paragraph.content} highlights={highlights} onHighlightClick={onOpen} className="leading-relaxed [&_table]:text-[1em] [&_code]:text-[.9em]"/></div>
    {!readOnly && (article ? <><button type="button" title={selection?'Annotate selected text':'Add annotation'} aria-label={`Add annotation to paragraph ${index+1}`} className="absolute -left-8 top-0 inline-flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-blue-50 hover:text-blue-700 focus-visible:outline-2 focus-visible:outline-blue-600" onClick={()=>onAnnotate(selection)}><svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7"><path d="m16 3 5 5-12 12-6 1 1-6L16 3Z"/><path d="m14 5 5 5"/></svg></button>{selection&&<div className="mt-2 font-sans">{actions}</div>}</> : actions)}
  </section>;
}
