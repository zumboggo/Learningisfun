...
function ParagraphCard({ article, paragraph, index, label, readOnly, onAnnotate, onHighlight }: { article:boolean; paragraph:TextParagraph;index:number;label?:string;readOnly:boolean;onAnnotate:(selection:string)=>void;onHighlight:(selection:string)=>void }) {
  ...
  return <section aria-label={`Paragraph ${index+1}`} className={article ? 'space-y-2' : 'space-y-3 rounded-xl border bg-white p-4 text-slate-900'}>
    {(!article || label) && <p className="text-xs font-medium text-gray-500">{label? `${label} · `:''}Paragraph {index+1}</p>}
