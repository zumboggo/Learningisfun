...
describe('planner source parser',()=>{
 ...
 it('creates quiz-result and presentation-to-flashcard tasks for each class section',()=>{const week=parsePlannerSource(source).weeks[0],plan=createWeeklyPlan(week,{});expect(plan.preparation.map(task=>task.label)).toEqual(['Update Quiz results in Canvas · World Literature · Blue','Add flashcards to World Literature · Blue · Kennings','Update Quiz results in Canvas · World Literature · Red','Add flashcards to World Literature · Red · Kennings']);expect(plan.courses[0].texts.map(text=>text.title)).toEqual(['Beowulf opening']);expect(plan.courses[0].presentations.map(item=>item.title)).toEqual(['Kennings']);});
});
