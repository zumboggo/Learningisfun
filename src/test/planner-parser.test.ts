...
describe('planner source parser',()=>{
 ...
 it('creates presentation preparation without automatic flashcard tasks',()=>{const week=parsePlannerSource(source).weeks[0],plan=createWeeklyPlan(week,{});expect(plan.preparation.map(task=>task.label)).toEqual(['Prepare Presentation · World Lit']);expect(plan.courses[0].texts.map(text=>text.title)).toEqual(['Beowulf opening']);expect(plan.courses[0].presentations.map(item=>item.title)).toEqual([]);});
});
