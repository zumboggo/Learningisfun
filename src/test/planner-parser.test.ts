...
describe('planner source parser',()=>{
 ...
 it('uses one flashcard readiness task per class without duplicating content choices in preparation',()=>{const week=parsePlannerSource(source).weeks[0],plan=createWeeklyPlan(week,{});expect(plan.preparation.map(task=>task.label)).toEqual(['Flashcards updated · World Literature · Blue','Flashcards updated · World Literature · Red']);expect(plan.courses[0].texts.map(text=>text.title)).toEqual(['Beowulf opening']);expect(plan.courses[0].presentations.map(item=>item.title)).toEqual(['Kennings']);});
});
