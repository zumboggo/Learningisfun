...
describe('planner source parser',()=>{
 ...
 it('creates one flashcard task and presentation preparation for each section',()=>{const week=parsePlannerSource(source).weeks[0],plan=createWeeklyPlan(week,{});expect(plan.preparation.map(task=>task.label)).toEqual(['Flashcards Updated','Prepare Presentation · World Literature · Blue','Prepare Presentation · World Literature · Red']);expect(plan.courses[0].texts.map(text=>text.title)).toEqual(['Beowulf opening']);expect(plan.courses[0].presentations.map(item=>item.title)).toEqual(['Kennings']);});
});
