...
export function createWeeklyPlan(week:PlannerWeekSource,mapping:Record<string,string>,previous?:WeeklyPlanData):WeeklyPlanData{
 ...
 const preparation:PreparationTask[]=[];const cardGroups=new Map<string,{label:string;classCode:string}>();for(const block of week.blocks){const key=block.code.startsWith('WL-')?'WL':block.code;if(!cardGroups.has(key))cardGroups.set(key,{label:key==='WL'?'World Literature':block.label,classCode:block.code});}for(const [key,group] of cardGroups)preparation.push({id:`cards-${key}`,label:`Flashcards updated · ${group.label}`,kind:'quiz',status:'todo',classCode:group.classCode});
