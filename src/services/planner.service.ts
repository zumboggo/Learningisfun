...
export interface WeeklyPlanData { cancelledLessons?: LessonPlan[]; lessonRequirements?: { qualities?: string; eal?: string; learning?: string }; unitSnapshot?: UnitPlan[]; cardEditorVersion?: number; dismissedAssignedTexts?: string[]; weeklyResources?: import('./planner-bank').WeeklyResource[] }
...
export function createWeeklyPlan(week:PlannerWeekSource,mapping:Record<string,string>,previous?:WeeklyPlanData):WeeklyPlanData{
 ...
 const preparation:PreparationTask[]=[...week.blocks.map(block=>({id:`prepare-presentation-${block.code}`,label:`Prepare Presentation · ${block.label}`,kind:'presentation' as const,status:'todo' as const,classCode:block.code}))];
 return normalizePlan({week,flags:[],weekNote:'',preparation,courses,extras:[],lessons,publishAgenda:true,includeIntentionsInPrint:false});
}
