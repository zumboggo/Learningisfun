...
export function createWeeklyPlan(week:PlannerWeekSource,mapping:Record<string,string>,previous?:WeeklyPlanData):WeeklyPlanData{
 ...
 const preparation:PreparationTask[]=[];for(const block of week.blocks){for(const title of block.presentationCandidates)preparation.push({id:`pres-${block.code}-${title}`,label:`Prepare presentation: ${title}`,kind:'presentation',status:'todo',classCode:block.code});for(const day of block.days){if(day.C.toLowerCase().includes('quiz'))preparation.push({id:`quiz-${block.code}-${day.iso}`,label:`Check quiz for ${block.label} · ${day.date}`,kind:'quiz',status:'todo',classCode:block.code});for(const due of day.due)preparation.push({id:`due-${block.code}-${day.iso}-${due}`,label:`Remind students: ${due}`,kind:'assignment',status:'todo',classCode:block.code,dueAt:day.iso});}}
