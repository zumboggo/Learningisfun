...
export function WeeklyPlannerEditor({ initial, suggestedWeek, record, sourceId, userId, units, onUnitsChange, onSaved }: { initial: WeeklyPlanData; suggestedWeek?: PlannerWeekSource; record?: WeeklyPlanRecord; sourceId: string; userId: string; units: UnitPlan[]; onUnitsChange: (units: UnitPlan[]) => void; onSaved: (record: WeeklyPlanRecord) => void }) {
  ...
  return <div className="space-y-5">
    ...
    <fieldset disabled={busy||Boolean(share)} className="min-w-0 space-y-5">
      ...
      <section className="rounded-xl border bg-white p-3"><h2 className="mb-2 text-sm font-semibold">Lesson-plan requirements</h2><p className="mb-3 text-xs text-slate-500">Short weekly notes, included in the printed summary. Private planning only.</p>{([['qualities','9 qualities'],['eal','EAL Support'],['learning','Learning Support']] as const).map(([key,label])=><label key={key} className="mb-2 grid gap-1 text-xs sm:grid-cols-[130px_1fr] sm:items-center">{label}<input maxLength={180} className="rounded border px-2 py-1.5 text-sm" value={data.lessonRequirements?.[key]||''} onChange={event=>change({...data,lessonRequirements:{...data.lessonRequirements,[key]:event.target.value}})}/></label>)}</section>
    </fieldset>
