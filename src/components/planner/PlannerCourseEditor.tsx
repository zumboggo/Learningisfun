...
export function PlannerCourseEditor({ blocks, data, mutate }: { blocks: PlannerBlockSource[]; data: WeeklyPlanData; mutate: Change }) {
  ...
  const syncPresentation = (oldTitle: string, value: Partial<PresentationChoice>) => mutate(draft => {
    draft.courses.filter(course => codes.includes(course.classCode)).forEach(course => {
      ...
      const first = course.presentations[0], task = draft.preparation.find(item => item.id === `add-cards-${course.classCode}`);
      if (task && first) { const block=blocks.find(item=>item.code===course.classCode);task.label=`Add flashcards to ${block?.label||course.classCode} · ${first.title}`;task.url=first.url||undefined; }
    });
  });
  const placeActivity = (label: string, target: string | undefined, lessonId: string) => mutate(draft => {
    const lesson = draft.lessons.find(item => item.id === lessonId); if (!lesson) return;
    const matching = draft.extras.find(item => item.courseCode === lesson.classCode && item.label === label && item.target === target); if (!matching) return;
    if (!matching.lessonDates.includes(lesson.date)) matching.lessonDates.push(lesson.date);
    if (!lesson.extraActivityIds.includes(matching.id)) lesson.extraActivityIds.push(matching.id);
  });
  const dropActivity = (event: DragEvent<HTMLDivElement>, lessonId: string) => {
    event.preventDefault();
    try { const item=JSON.parse(event.dataTransfer.getData('application/x-planner-activity'));placeActivity(item.label,item.target,lessonId); }
  ...
  return <Card>
    ...
    <section className="mt-5">
      <h4 className="font-semibold">Activities aimed at these targets</h4>
      <p className="mt-1 text-xs text-gray-500">Drag an activity to a class period below. Copy its text when you want a similar version.</p>
      <div className="mt-2 space-y-2">{extras.map(extra => <div key={`${extra.label}-${extra.target}`} draggable onDragStart={event=>event.dataTransfer.setData('application/x-planner-activity',JSON.stringify({label:extra.label,target:extra.target}))} className="flex cursor-grab items-start justify-between gap-2 rounded-lg bg-gray-100 px-3 py-2 text-sm active:cursor-grabbing"><span>{extra.label}{extra.target && <small className="mt-1 block text-blue-700">Targets: {extra.target}</small>}</span><span className="flex gap-2"><button className="text-blue-700" title="Copy activity text" onClick={()=>void navigator.clipboard.writeText(extra.label)}>⧉</button><button className="text-red-600" title="Remove activity" onClick={() => mutate(draft => { const removed=draft.extras.filter(item=>codes.includes(item.courseCode)&&item.label===extra.label&&item.target===extra.target).map(item=>item.id);draft.extras=draft.extras.filter(item=>!removed.includes(item.id));draft.lessons.forEach(lesson=>{lesson.extraActivityIds=lesson.extraActivityIds.filter(id=>!removed.includes(id))}); })}>×</button></span></div>)}</div>
      <div className="mt-2 grid gap-2 sm:grid-cols-[1fr_1fr_auto]"><input className="rounded-lg border px-3 py-2 text-sm" placeholder="Add or paste an activity" value={activity} onChange={event => setActivity(event.target.value)} /><select className="rounded-lg border px-3 py-2 text-sm" value={activityTarget} onChange={event => setActivityTarget(event.target.value)}><option value="">All weekly targets</option>{weeklyTargets.map((target, index) => <option key={target} value={`${index + 1}. ${target}`}>Target {index + 1}: {target}</option>)}</select><Button size="sm" variant="secondary" disabled={!activity.trim()} onClick={() => { mutate(draft => codes.forEach(code => draft.extras.push({ id: crypto.randomUUID(), courseCode: code, label: activity.trim(), target: activityTarget || 'All weekly targets', lessonDates: [] }))); setActivity(''); setActivityTarget(''); }}>Add</Button></div>
      <div className="mt-3 grid gap-2 sm:grid-cols-2">{data.lessons.filter(lesson=>codes.includes(lesson.classCode)).map(lesson=><div key={lesson.id} onDragOver={event=>{event.preventDefault();event.dataTransfer.dropEffect='copy'}} onDrop={event=>dropActivity(event,lesson.id)} className="min-h-16 rounded-xl border-2 border-dashed border-blue-200 bg-blue-50/50 p-2"><p className="text-xs font-bold text-blue-800">{lesson.classLabel} · {lesson.date}</p><div className="mt-1 flex flex-wrap gap-1">{data.extras.filter(item=>lesson.extraActivityIds.includes(item.id)).map(item=><button key={item.id} title="Remove from this period" onClick={()=>mutate(draft=>{const current=draft.lessons.find(row=>row.id===lesson.id);if(current)current.extraActivityIds=current.extraActivityIds.filter(id=>id!==item.id)})} className="rounded bg-white px-2 py-1 text-left text-xs shadow-sm">{item.label} ×</button>)}</div>{lesson.extraActivityIds.length===0&&<p className="mt-2 text-xs text-blue-500">Drop activity here</p>}</div>)}</div>
    </section>
