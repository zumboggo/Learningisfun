  ...
  return <aside aria-label="Planning resources" className="min-w-0 space-y-3 rounded-2xl border border-slate-200 bg-white p-3 lg:sticky lg:top-32 lg:max-h-[calc(100dvh-9rem)] lg:overflow-y-auto">
    ...
    <div className="space-y-3">
      {(['presentation','routine','text','activity'] as const).filter(kind=>category==='all'||kind===category).map(kind=><section key={kind} aria-label={kind+' resources'} className={'min-w-0 rounded-xl border p-2 '+(kind==='presentation'?'border-purple-200 bg-purple-50':kind==='text'?'border-blue-200 bg-blue-50':kind==='routine'?'border-amber-200 bg-amber-50':'border-emerald-200 bg-emerald-50')}>
        ...
        {kind==='presentation'&&onUpload&&<button className="mb-2 w-full rounded-lg border border-purple-300 bg-white px-3 py-2 text-sm font-semibold text-purple-900" onClick={onUpload}>+ PowerPoint</button>}{kind==='activity'&&!items.some(isQuiz)&&<button className="mb-2 block w-full rounded-lg border border-rose-300 bg-rose-50 p-2 text-left text-sm font-semibold text-rose-950" onClick={()=>onAdd('quiz')}>+ Quiz</button>}<div className="space-y-1">{visibleItems(kind).filter(item=>kind!=='routine'||primaryRoutine(item)).map(renderItem)}</div>
        {kind==='routine'&&visibleItems(kind).some(item=>!primaryRoutine(item))&&<details className="mt-2" open={weeklySearch.trim()?true:undefined}><summary className="min-h-9 cursor-pointer py-2 text-xs font-medium">Routines · {visibleItems(kind).filter(item=>!primaryRoutine(item)).length} more</summary><div className="space-y-1">{visibleItems(kind).filter(item=>!primaryRoutine(item)).map(renderItem)}</div></details>}
      </section>)}
    </div>
