  ...
  return <section className="space-y-3" onDragEnd={()=>setDragTarget('')}>
    ...
    <div className="space-y-4">
      ...
      <div className={'grid min-w-0 items-start gap-3 md:grid-cols-2 '+(compare?'':'2xl:grid-cols-3')}>{displayedLessons.map((lesson,position) => lesson ? <article key={lesson.id} aria-label={lesson.date + ' ' + lesson.classLabel} onDragOver={e => e.preventDefault()} onDrop={e => drop(e, lesson.id, lesson.slots?.length || 0)} className="rounded-xl border bg-white p-2">
        ...
        <div className="mt-2 space-y-1.5">{(lesson.slots || []).map((slot,index)=>({slot,index})).sort((a,b)=>Number(isAssignedReading(b.slot))-Number(isAssignedReading(a.slot))).map(({slot,index}) => <div key={slot.id} draggable onDragStart={e => { e.dataTransfer.setData(dragType, JSON.stringify({ slot, from: lesson.id, index })); e.dataTransfer.effectAllowed = 'move'; }} onDragOver={e => {e.preventDefault();e.stopPropagation();setDragTarget(lesson.id+':'+index);}} onDrop={e => drop(e, lesson.id, index)} className={'flex h-16 cursor-grab items-center gap-1 rounded-xl border px-2 py-1 ' + resourceColor(slot) + (dragTarget===lesson.id+':'+index?' border-t-4 border-t-blue-600':'')}>
          <button className="min-w-0 flex-1 text-left text-sm" aria-label={selected ? 'Place before ' + slot.title : 'Open ' + slot.title + ' details'} onClick={() => selected ? place(lesson.id, index) : setPreview(slot)}><span className="block text-[10px] uppercase opacity-60">{isQuiz(slot)?'':resourceCategory(slot)==='text'?(isAssignedReading(slot)?'Assigned reading':'Text'):resourceCategory(slot)}</span><span className="block truncate text-xs font-semibold">{isCopywork(slot)&&<span aria-label="Copywork" className="mr-1 rounded border px-1 font-bold">C</span>}{shortWords(resourceTitle(slot),6)}</span>{!isQuiz(slot) && slot.content && <span className="block truncate text-[11px] opacity-70">{shortWords(slot.content,12)}</span>}</button>
