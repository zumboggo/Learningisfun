...
export function WeeklySlots({ data: suppliedData, units, onChange, preferenceKey }: { data: WeeklyPlanData; units: UnitPlan[]; onChange: (data: WeeklyPlanData) => void; onUnitsChange?: (units: UnitPlan[]) => void; preferenceKey?: string }) {
  ...
  return <section className="space-y-3" onDragEnd={()=>setDragTarget('')}>
    ...
    <Modal open={Boolean(pickingLesson)} onClose={()=>setPickingLesson(null)} title="Add to a lesson">{pickingLesson&&<div className="space-y-3"><p className="text-sm font-medium">{pickingLesson.title}</p><p className="text-xs text-slate-500">Choose a lesson this week. The item will be added at the end; its details stay linked to the planning area.</p><div className="max-h-[60vh] space-y-2 overflow-y-auto">{data.lessons.filter(lesson=>courseCode(lesson.classCode)===((data.weeklyResources||[]).find(item=>item.id===pickingLesson.planningItemId)?.course||courseCode(code))).sort((a,b)=>a.date.localeCompare(b.date)||a.classCode.localeCompare(b.classCode)).map(lesson=><button key={lesson.id} className="block w-full rounded-xl border px-3 py-3 text-left hover:bg-blue-50 focus-visible:ring-2 focus-visible:ring-blue-600" onClick={()=>{place(lesson.id,lesson.slots?.length||0,{slot:pickingLesson});setMessage('Added '+pickingLesson.title+' to '+(plannerLabel[lesson.classCode]||lesson.classLabel)+' · '+lesson.date);setPickingLesson(null);}}><span className="block text-sm font-semibold">{plannerLabel[lesson.classCode]||lesson.classLabel}</span><span className="text-xs text-slate-600">{new Date(lesson.date+'T12:00:00').toLocaleDateString('en-US',{weekday:'long',month:'short',day:'numeric'})}{lesson.slots?.some(slot=>slot.planningItemId===pickingLesson.planningItemId)?' · Already included — add another copy':''}</span></button>)}</div>{!data.lessons.some(lesson=>courseCode(lesson.classCode)===courseCode(code))&&<p className="text-sm text-slate-500">No lessons scheduled for this course this week.</p>}<button className="rounded-lg border px-3 py-2 text-sm" onClick={()=>setPickingLesson(null)}>Cancel</button></div>}</Modal>
