...
export function WeeklySlots({ data: suppliedData, units, onChange, preferenceKey }: { data: WeeklyPlanData; units: UnitPlan[]; onChange: (data: WeeklyPlanData) => void; onUnitsChange?: (units: UnitPlan[]) => void; preferenceKey?: string }) {
  ...
  return <section className="space-y-3" onDragEnd={()=>setDragTarget('')}>
    ...
    <div data-testid="planning-desk" className="grid items-start gap-4 lg:grid-cols-[minmax(18rem,0.85fr)_minmax(0,1.6fr)]">
      <PlannerResourceTray courseLabel={code.startsWith('WL-')?'World Lit Blue + Red resource bank':(plannerLabel[code]||code)+' resource bank'} resources={resources} week={data.week.startDate} lessons={data.lessons.filter(l=>courseCode(l.classCode)===courseCode(code))} items={(data.weeklyResources||[]).filter(item=>item.course===courseCode(code))} onSelect={selectResource} onAdd={addResource} onAddRoutine={()=>setChoosingRoutine(true)} onEdit={setEditing} onPlace={slot=>{setSelected(null);setPickingLesson(slot);}} onDelete={removeResource}/>
