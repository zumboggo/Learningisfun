...
export function WeeklySlots({ data: suppliedData, units, onChange, preferenceKey }: { data: WeeklyPlanData; units: UnitPlan[]; onChange: (data: WeeklyPlanData) => void; onUnitsChange?: (units: UnitPlan[]) => void; preferenceKey?: string }) {
  ...
  const addResource = (kind:ResourceKind, routine?:typeof routineNames[number] | 'Custom activity') => {
    if(kind==='activity' && !routine){setChoosingRoutine(true);return;}
    const slot=routine && routine!=='Custom activity' ? routineSlot(routine) : resourceSlot({...blankResource(),kind,title:'New '+kind,week:data.week.startDate});
  ...
  return <section className="space-y-3" onDragEnd={()=>setDragTarget('')}>
    ...
    <div className="space-y-4">
      <PlannerResourceTray resources={resources} week={data.week.startDate} lessons={data.lessons.filter(l=>courseCode(l.classCode)===courseCode(code))} items={(data.weeklyResources||[]).filter(item=>item.course===courseCode(code))} onSelect={selectResource} onAdd={addResource} onEdit={setEditing} onPlace={slot=>{setSelected(null);setPickingLesson(slot);}} onDelete={removeResource}/>
      <div className={'grid min-w-0 items-start gap-3 md:grid-cols-2 '+(compare?'':'2xl:grid-cols-3')}>{displayedLessons.map((lesson,position) => lesson ? <article key={lesson.id} aria-label={lesson.date + ' ' + lesson.classLabel} onDragOver={e => e.preventDefault()} onDrop={e => drop(e, lesson.id, lesson.slots?.length || 0)} className="rounded-xl border bg-white p-2">
        ...
        <div className="mt-2 space-y-1.5">{(lesson.slots || []).map((slot, index) => <div key={slot.id} draggable onDragStart={e => { e.dataTransfer.setData(dragType, JSON.stringify({ slot, from: lesson.id, index })); e.dataTransfer.effectAllowed = 'move'; }} onDragOver={e => {e.preventDefault();e.stopPropagation();setDragTarget(lesson.id+':'+index);}} onDrop={e => drop(e, lesson.id, index)} className={'flex h-16 cursor-grab items-center gap-1 rounded-xl border px-2 py-1 ' + color(slot.kind) + (dragTarget===lesson.id+':'+index?' border-t-4 border-t-blue-600':'')}>
          <button className="min-w-0 flex-1 text-left text-sm" aria-label={selected ? 'Place before ' + slot.title : 'Open ' + slot.title + ' details'} onClick={() => selected ? place(lesson.id, index) : setPreview(slot)}><span className="block text-[10px] uppercase opacity-60">{slot.kind === 'activity' ? slot.activityType || slot.kind : slot.kind}</span><span className="block truncate text-xs font-semibold">{shortWords(slot.title,6)}</span>{slot.content && <span className="block truncate text-[11px] opacity-70">{shortWords(slot.content,12)}</span>}</button>
    ...
    <Modal open={choosingRoutine} onClose={()=>setChoosingRoutine(false)} title="Add activity"><div className="space-y-3"><p className="text-sm text-slate-600">Choose a routine or create your own activity.</p><div className="grid grid-cols-2 gap-2">{[...routineNames,'Custom activity' as const].map(name=><button key={name} className="rounded-lg border px-3 py-3 text-left text-sm hover:bg-emerald-50" onClick={()=>addResource('activity',name)}>{name}</button>)}</div></div></Modal>
