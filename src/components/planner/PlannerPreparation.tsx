...
import { plannerLabel, progressLabels } from '@/services/planner-layout';
...
export function PlannerPreparation({ data, units, onChange }: { data: WeeklyPlanData; units: UnitPlan[]; onChange: (data: WeeklyPlanData) => void }) {
  ...
  return <>
    <section className="space-y-3 rounded-2xl border bg-white p-4" aria-label="Weekly to-do">
      ...
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{data.week.blocks.map(block => {
        const task = data.preparation.find(item => item.id === `prepare-presentation-${block.code}`);
        if (!task) return null;
