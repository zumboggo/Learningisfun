...
import { plannerLabel, preparationCode } from '@/services/planner-layout';
  ...
  return <>
    <section className="space-y-3 rounded-2xl border bg-white p-4" aria-label="Weekly to-do">
      ...
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{data.week.blocks.filter((block, index, blocks) => blocks.findIndex(item => preparationCode(item.code) === preparationCode(block.code)) === index).map(block => {
        ...
        return <details key={block.code} className={`rounded-xl border p-3 ${task.status === 'ready' ? 'border-emerald-200 bg-emerald-50' : 'border-purple-200 bg-purple-50/40'}`}><summary className="cursor-pointer text-sm"><strong className="block">{code === 'WL' ? 'World Lit · Blue + Red' : plannerLabel[block.code] || block.label}</strong><span>Create presentation {task.status === 'ready' ? '✓' : ''}</span></summary><div className="mt-3 space-y-3 text-sm"><CopyButton label="Copy prompt" text={`Create a /vocab-presentation for ${code==='WL'?'World Literature':block.label} this week.`}/><p className="font-medium">{(topics.length ? topics : fallback).join(' · ') || block.unit || 'Choose a presentation topic'}</p><p className="text-slate-600">{block.goal}</p><div className="flex flex-wrap gap-1">{cards.map(card => <span key={card.id} title={card.back} className="rounded bg-white px-2 py-1 text-xs">{card.front}</span>)}</div>{!cards.length && <p className="text-xs text-slate-500">No vocabulary scheduled for this week.</p>}<label className="flex items-start gap-2"><input type="checkbox" checked={task.status === 'ready'} onChange={e => check(task.id, e.target.checked)}/>Presentation done and link posted</label></div></details>;
      })}</div>
