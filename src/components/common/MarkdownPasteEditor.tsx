...
export function MarkdownPasteEditor({ value, onChange, rows = 10, placeholder, autoFocus = false, preview = true }: { value:string;onChange:(value:string)=>void;rows?:number;placeholder?:string;autoFocus?:boolean;preview?:boolean }) {
  ...
  const replaceSelection = (before:string,after=before,sample='text') => {
    ...
    const selected=value.slice(start,end)||sample; const next=`${value.slice(0,start)}${before}${selected}${after}${value.slice(end)}`;
  ...
  return <div className="space-y-2"><div className="flex flex-wrap gap-1 rounded-t-lg border border-b-0 bg-gray-50 p-2"><Tool label="B" className="font-bold" onClick={()=>replaceSelection('**','**','bold text')}/><Tool label="I" className="italic" onClick={()=>replaceSelection('*','*','italic text')}/><Tool label="Heading" onClick={()=>replaceSelection('## ','','Heading')}/><Tool label="• List" onClick={()=>replaceSelection('- ','','List item')}/><Tool label="Quote" onClick={()=>replaceSelection('> ','','Quoted text')}/><Tool label="Link" onClick={()=>replaceSelection('[','](https://)','link text')}/></div><textarea ref={ref} autoFocus={autoFocus} rows={rows} className="w-full rounded-b-lg border px-3 py-3 font-mono text-sm leading-6" placeholder={placeholder} value={value} onChange={event=>onChange(event.target.value)} onPaste={paste}/><p className="text-xs text-gray-500">Paste from a webpage or document to keep headings, bold, italics, lists, links, quotes, and tables where possible.</p>{preview&&<div className="rounded-lg border bg-white p-4"><p className="mb-3 text-xs font-semibold uppercase tracking-wide text-gray-400">Preview</p><Markdown content={value||'*Nothing pasted yet.*'} className="text-base text-gray-800"/></div>}</div>;
}
