...
export function EditReadingContribution({post,busy,onSave,onCancel}:{post:ReadingDiscussionPost;busy:boolean;onSave:(fields:Record<string,unknown>)=>Promise<void>;onCancel:()=>void}){
 ...
 {(post.parentId||post.quotation)&&<><label className="block text-sm">Quotation<textarea placeholder="Paste quote here" className="mt-1 w-full rounded border p-2" maxLength={3000} value={quotation} onChange={e=>setQuotation(e.target.value)}/></label>
