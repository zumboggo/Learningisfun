...
function TextEditorForm({text,teacherId,classes,paragraphs,assignments,onClose}:{text:LearningText;teacherId:string;classes:Array<{id:string;name:string}>;paragraphs:TextParagraph[];assignments:TextAssignment[];onClose:()=>void}){
  const[title,setTitle]=useState(text.title),[author,setAuthor]=useState(text.author),[externalUrl,setExternalUrl]=useState(text.externalUrl||'');
  ...
  const [purposes,setPurposes]=useState<Record<string,TextPurpose>>(Object.fromEntries(assignments.map(a=>[a.classId,{isCopywork:a.isCopywork,isAssignedReading:a.isAssignedReading}])));
  ...
  const valid=Boolean(title.trim())&&(!externalUrl.trim()||/^https?:\/\//i.test(externalUrl.trim()))&&(text.contentMode==='link'?(!externalUrl.trim()||/^https?:\/\//i.test(externalUrl)):parsed.length>0);
  const save=async()=>{setBusy(true);setError('');try{if(text.contentMode!=='link')await updateTextParagraphs(text.$id,teacherId,parsed);await updateTextMetadata(text.$id,teacherId,{title,author,source:text.source,externalUrl});await updateTextAssignments(text.$id,teacherId,[...chosen].map(classId=>({classId,...purposes[classId],...textSchedule(dates[classId]||'',assignments.find(row=>row.classId===classId&&!row.dueDate)?.assignedAt)})));onClose()}catch(cause){setError(cause instanceof Error?cause.message:'Could not save this text.')}finally{setBusy(false)}};
  const saveButton=<Button className="w-full" loading={busy} disabled={!valid} onClick={()=>void save()}>Save all changes</Button>;
  return <Modal open onClose={()=>!busy&&onClose()} title="Edit text"><div className="max-h-[78vh] space-y-4 overflow-y-auto pr-1">{error&&<p className="rounded-lg bg-red-50 p-3 text-sm text-red-700">{error}</p>}<div className="grid gap-3 sm:grid-cols-2"><label className="text-sm font-medium sm:col-span-2">Title<input className={input} value={title} onChange={event=>setTitle(event.target.value)}/></label><label className="text-sm font-medium">Author<input className={input} value={author} onChange={event=>setAuthor(event.target.value)}/></label></div><section className="space-y-2"><div><h3 className="font-semibold">Class access and reading date</h3><p className="text-xs text-gray-500">Uncheck a class to remove access. Each class can have its own date.</p></div>{classes.map(cls=><div key={cls.id} className="flex flex-wrap justify-between items-center gap-3 rounded-lg border p-3"><label className="flex min-w-0 items-center gap-3 text-sm font-medium"><input type="checkbox" className="h-4 w-4" checked={chosen.has(cls.id)} onChange={()=>toggle(cls.id)}/><span className="truncate">{cls.name}</span></label><ClassReadingDate purpose={purposes[cls.id]} onPurposeChange={value=>setPurposes(current=>({...current,[cls.id]:value}))} name={cls.name} value={dates[cls.id]||''} disabled={!chosen.has(cls.id)} onChange={value=>setDates(current=>({...current,[cls.id]:value}))}/></div>)}</section>{text.contentMode==='link'?<label className="block text-sm font-medium">Source (optional link)<input type="url" className={input} value={externalUrl} onChange={event=>setExternalUrl(event.target.value)} placeholder="https://…"/></label>:<section><label className="mb-3 block text-sm font-medium">Source (optional link)<input type="url" className={input} value={externalUrl} onChange={event=>setExternalUrl(event.target.value)} placeholder="https://…"/></label><h3 className="mb-2 font-semibold">Text</h3>{saveButton}<MarkdownPasteEditor value={content} onChange={setRaw} rows={14}/><p className="mt-2 text-xs text-gray-500">{parsed.length} paragraph{parsed.length===1?'':'s'} · Existing paragraph positions are preserved so annotations stay attached. Annotated paragraphs cannot be removed.</p></section>}{text.contentMode==='link'&&saveButton}</div></Modal>;
}
